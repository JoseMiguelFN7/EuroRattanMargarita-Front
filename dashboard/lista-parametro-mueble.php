...
<head>
        ...
        <main class="ease-soft-in-out xl:ml-sidebar relative h-full max-h-screen rounded-xl transition-all duration-200">
          ...
          <section class="">
            <div class="md:mb-0 pt-6"> 
              <h2 class=" text-xxl font-medium text-center text-gray-600 md:text-stroke-brown">Lista de Tipos de Mueble</h2>
            </div>
          </section>
    ...
    <script>
      $(document).ready(function(){
        ...
        $('body').on('click', '.delete-ft', function(){
          Swal.fire({
            icon: 'warning',
            ...
            if (result.isConfirmed) {
              ...
              $.ajax({
                url: `http://127.0.0.1:8000/api/furnitureType/${id}`,
                type: 'DELETE',
                dataType: 'json',
                headers: {
        ...
        $.ajax({
          ...
          success: function(response){
            ...
            response.forEach(furnitureType => {
            ...
            $('#tableBody').html('').append(ftTableBodyHTML);
          }
        });
      });
    </script>
